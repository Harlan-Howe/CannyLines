...
def load_smooth_and_grey_image()->None:
    ...
    This generates the following three global variables, each is an np.ndarray with
    ...
    smoothed_image = cv2.GaussianBlur(source_image, (5,5),0) # create a smoothed out version of this image to de-noise it.
    grey_image = cv2.cvtColor(smoothed_image,cv2.COLOR_BGR2GRAY) # convert this to black-and-white range 0 to 255 (ints)
...
def find_horizontal_differences(source: np.ndarray)-> np.ndarray:
    ...
    """
    result:np.ndarray = np.zeros(shape=source.shape, dtype=np.float )
...
def build_an_edge(magnitude_map, angle_map)->Edge:
    """
    ...
    consisting only of points that are at or above the STOP_THRESHOLD. If no points were at START_THRESHOLD, return None.
    ...
    # caculate a rounded off version of that point.
...
def add_edge_to_edge_list (edge_to_add: Edge)->None:
    ...
    • kept the same number of edges (added the new edge an to existing edge - e.g., list had edge A and adding new edge
    ...
    #  them into one edge and continue searching to see whether there are any other edges to join before addding.
...
def draw_all_edges(destination: np.ndarray, edge_list: List[Edge], line_thickness = 3)->None:
    """
    draws all the lines in edge_list onto destination, with each line a unique color.
    :param destination: the nd.nparray in which to draw
    :param edge_list: the list of Lines (i.e., lists of points)
        ...
        for p in edge:
            ...
            for i in range (-int(line_thickness/2), int(line_thickness/2+0.5)):
                for j in range (-int(line_thickness/2), int(line_thickness/2+0.5)):
                    if p[0]+i >= 0 and p[0]+i<source_image.shape[0] and p[1]+j>=0 and p[1]+j < source_image.shape[1]:
                        # only color worthwhile lines, but black out any we find (or we'll keep finding the old ones).
                        # if len(edge) > MIN_LENGTH:
                        destination[p[0]+i,p[1]+j] = (b,g,r)
...
if __name__ == '__main__':
    ...
    cv2.imshow("Sobel_mag original", change_magnitude_image/255) # shows the image (0-1.0) in a window
                                                                 #    called "Sobel_mag original."
